...
var ListenerHacker = {
  ...
  disableListeners: function (name, t) {
    let th = window.ListenerHacker;
    if (th.listenerFuncs[name][t] === undefined) {return}
    for (let i = 0; i < th.listenerFuncs[name][t].length; i++) {
      th.listenerElems[name].removeEventListener(...th.listenerFuncs[name][t][i])
    }
  },
  enableListeners: function (name, t) {
    ...
    if (th.listenerFuncs[name][t] === undefined) {return}
    for (let i = 0; i < th.listenerFuncs[name][t].length; i++) {
      th.origAddEventListener.call(th.listenerElems[name], ...th.listenerFuncs[name][t][i])
    }
  },
  isActivated: false,
  activate: function () {
  ...
  dispatchEvent: function (name, e) {
    let th = window.ListenerHacker;
    th.listenerFuncs[name][e.type].forEach((x) => {x[1].call(th.listenerElems[name], e)})
  },
